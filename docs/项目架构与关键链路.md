...
| 数据库迁移 | L0/L2 已验收 | `SchemaMigrationExecutor` 按唯一 manifest 校验增量脚本、批准基线、依赖和 ledger 状态；`JdbcMigrationStore` 对 PostgreSQL ledger catalog、事务执行和失败状态进行 fail-closed 管理。真实隔离 clean install/upgrade、前缀升级、重放和失败恢复已通过；生产发布入口与完整对象 catalog 对账仍待验收。 |
...
提交后 [RabbitKnowledgeBaseDeletionTaskPublisher.java](../backend_v2/src/main/java/com/kama/jchatmind/deletion/RabbitKnowledgeBaseDeletionTaskPublisher.java) 投递独立 queue。[KnowledgeBaseDeletionTaskConsumer.java](../backend_v2/src/main/java/com/kama/jchatmind/deletion/KnowledgeBaseDeletionTaskConsumer.java) 在单消费者、单预取下领取任务，调用 [DocumentStorageServiceImpl.java](../backend_v2/src/main/java/com/kama/jchatmind/service/impl/DocumentStorageServiceImpl.java) 仅删除 `document.storage.base-path/<kbId>` 下的规范化目录；目录不存在成功，越出基目录拒绝。失败的清理依次进入重试和死信，查询 `GET /api/knowledge-base-deletion-tasks/{taskId}` 只向任务 owner 返回状态。该实现已有 L0/L1 契约和单元覆盖，并已在 2026-08-30 的隔离 PostgreSQL/RabbitMQ L2 验证数据库级联、审计、文件清理和第二账号隔离；`KnowledgeBaseTabContent` 现提供二次确认删除按钮，Hook 会等待删除任务终态并刷新列表，`g1-runtime.spec.ts` 已加入对应 Edge 旅程；2026-08-31 的动态端口隔离全栈 Edge 删除旅程已通过，生产迁移入口仍待验收。
