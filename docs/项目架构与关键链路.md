...
sequenceDiagram
    ...
    A->>MSG: 保存 ASSISTANT / TOOL 消息
    A->>SSE: AI_GENERATED_CONTENT, AI_DONE
    EVT->>MSG: finally 中提取候选记忆
```

1. 前端 [AgentChatView.tsx](../ui/src/components/views/AgentChatView.tsx) 创建会话、提交用户消息，并连接 `/sse/connect/{chatSessionId}`。
...
5. [JChatMind.java](../backend_v2/src/main/java/com/kama/jchatmind/agent/JChatMind.java) 运行最多 20 步的 Think-Execute 循环。模型内部工具执行被关闭，工具由应用显式执行，便于审批、超时、审计和 SSE 反馈。
6. [SseServiceImpl.java](../backend_v2/src/main/java/com/kama/jchatmind/service/impl/SseServiceImpl.java) 以会话 ID 保存 `SseEmitter`，发送序列化 `SseMessage`；断开、超时或发送失败后清理连接。
...
- 每轮模型输出先持久化并通过 SSE 发出；若含工具调用，才进入执行阶段。
...
- MCP 的方案与后续双向集成见 [MCP 双向集成开发方案](plans/active/mcp.md)。
...
| 实时消息 | `SseController` | `AgentChatView` 的 `EventSource` |
...
- [RAGAS 风格指标维护说明](RAGAS风格指标维护说明.md)
- [RAG 评测与数据集规范化推进计划](plans/active/rag-evaluation-dataset-governance-plan.md)
- [regression-v1 候选集人工审核清单](plans/active/regression-v1-candidate-review.md)
...
| `docs/reference/` | 稳定专题架构、可复用知识 | 一次性故障排查过程。 |
| `docs/spec/` | 开发前的范围、验收标准、数据 schema | 实施后的过程日志。 |
| `docs/plans/active/` | 尚在推进的方案、风险、下一步 | 已被完成实现替代的长期事实。 |
| `docs/plans/done/` | 已完成方案的目标与验收记录 | 新增需求的实施计划。 |
| `docs/records/` | 按日期记录故障、基线、决策和复盘 | 当前架构的唯一说明。 |

### 9.2 必须同步更新的触发条件

| 代码变化 | 最低文档动作 |
| --- | --- |
| 新增/删除 Controller 或变更请求契约 | 更新本文件第 6 节和对应专题文档。 |
| 改变 Agent 循环、工具执行或 Harness | 更新第 4.1、4.2、4.6 节，并新增 `records/` 记录。 |
| 改变 chunk、embedding、检索通道、rerank 或 query rewrite | 更新第 4.3、4.4、7 节，更新 RAG 数据集版本/评测报告。 |
| 改变记忆 schema、候选状态或召回逻辑 | 更新第 4.5 节、`sql/user-memory/README.md` 和相应迁移说明。 |
| 增加外部服务或环境变量 | 更新根 README 的启动说明和第 5 节；不得把敏感值写进文档。 |
| 完成 active 计划 | 在计划中记录验收结果，更新本文件的“已实现能力”，再移动到 `plans/done/`。 |

### 9.3 推荐检查顺序

1. 新成员：本文件 -> 根 [README](../README.md) -> 对应 Controller/Service/测试。
2. RAG 改动：本文件第 4.3、4.4、7 节 -> RAG 治理计划 -> 相关 `rag` 测试。
3. 生产故障：先建 `docs/records/<topic>/<date>-<event>.md`，修复稳定后再把持续有效的结论回填本文件或 `reference/`。

## 10. 关联文档

- [RAG 设计与评测分析](RAG设计与评测分析.md)：RAG 合理性、指标、多 Agent 和记忆演进建议。
- [RAGAS 风格指标维护说明](RAGAS风格指标维护说明.md)：指标实现、L1 验证和解释边界。
- [RAG 评测框架架构设计](reference/RAG评测框架架构设计.md)：历史三层评测设计。
- [RAG 优化收口与推进规范](reference/ai/rag-governance.md)：RAG 调优与收口规则。
- [RAG 评测与数据集规范化推进计划](plans/active/rag-evaluation-dataset-governance-plan.md)：当前评测数据治理计划。
- [用户记忆系统](plans/active/user-memory.md) 与 [记忆系统改进方案](plans/active/memory-system-improvement.md)：记忆设计与待办。
- [Agent Harness MVP 开发方案](plans/done/harness.md)：Harness 实施背景。
- [事件记录索引](records/README.md)：历史故障、基线和阶段结论。
