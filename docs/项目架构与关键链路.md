...
flowchart LR
    ...
    RB --> RR["rule rerank + bounded rank penalty"]
...
4. 在 RRF 和 `HARD` 过滤后只对前 50 个候选执行规则 rerank；候选排名 penalty 封顶为 `0.15`，预算外候选保留原 RRF 顺序，再输出最终 Top-K。
...
当前还有四个 G2 前的行为边界：未传 `kbIds` 时会话上次命中的 KB 会隐式收窄搜索范围，导致跨 KB topic switch 与“默认搜索全部可访问 KB”的语义需要明确取舍；短新实体可能被误判为 follow-up 而关闭标题通道，任意 `/`/`\\` 也可能被误判为导航并扫描 title/path 候选；同源通道去重/校准尚未实现；任何非空 Top-1 都会覆写 session retrieval context，低相关结果可污染下一轮。G2 的修复必须由范围、改写、rerank 和连续会话的回归用例共同签收，不能只调权重。
